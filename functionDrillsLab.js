...
function add(argument1, argument2) {
  const arg1 = parseInt(argument1)
  const arg2 = parseInt(argument2)
  return arg1 + arg2
}
...
/*
  ...
  If the name parameter is anything else, return 'Cool name, 
  //NAMEPARAM' (with NAMEPARAM being the value of the name parameter 
  //being passed in).
...
let colorRating = faceColorFinder("pink")
...
//CODE HERE
boneNames = ['Alina', 'Mal', 'Jesper', 'Alexander']
printAllNames = (boneNames) => {
  for (let idx in boneNames) {
    console.log(boneNames[idx])
  }
}
printAllNames(boneNames)
...
/*
  Write a function that takes in one argument, a string. The function should then 
  //console.log that string, in entirely uppercase characters.
...
let whichForceUser = darkForceUser("Kylo")
...
emailCheck = (email) => {
  ...
  const newemailCheck = emailCheck.trim()
  let validEmail = '@'
...
const totalFrogs = wizardBuys(33)
...
const totalFrogs2 = wizardBuys2(38)
...
/*
  ...
  Given the functions and variables above, edit the arrays below to contain only the
  appropriate variable names (as strings).
*/
